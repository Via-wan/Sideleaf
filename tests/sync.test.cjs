...
test('札记刷新带回空或旧阅读线时不会抹掉峥已有的位置', () => {
  const storage = new MemoryStorage({
    'sideleaf.reading-lines.v1': JSON.stringify({
      'book-1':{
        wish:{ current:88 },
        zheng:{ current:2304, furthest:2304, updatedAt:20 },
        schemaVersion:2,
        updatedAt:20
      }
    })
  ...
  SideleafSync.applyServerState(storage, {
    readingLines:{ 'book-1':{ zheng:{ current:null, furthest:null, updatedAt:21 } } },
    journals:[{
      id:'journal-1', bookId:'book-1', chapterKey:'1:0:第一章',
      chapterTitle:'第一章', text:'第一篇札记。', updatedAt:21
    }]
  });
  SideleafSync.applyServerState(storage, {
    readingLines:{ 'book-1':{ zheng:{ current:120, furthest:120, updatedAt:2 } } }
  ...
  const lines = JSON.parse(storage.getItem('sideleaf.reading-lines.v1'));
  assert.equal(lines['book-1'].wish.current, 88);
  assert.equal(lines['book-1'].zheng.current, 2304);
  assert.equal(lines['book-1'].zheng.furthest, 2304);
  assert.equal(JSON.parse(storage.getItem('sideleaf.chapter-journals.v1'))[0].text, '第一篇札记。');
});

test('更新更晚的有效书签仍可回退当前页，同时保留最远阅读线', () => {
  const storage = new MemoryStorage({
    'sideleaf.reading-lines.v1': JSON.stringify({
      'book-1':{ zheng:{ current:2304, furthest:2304, updatedAt:20 }, updatedAt:20 }
    })
  });

  SideleafSync.applyServerState(storage, {
    readingLines:{ 'book-1':{ zheng:{ current:120, furthest:120, updatedAt:21 } } }
  });

  const line = JSON.parse(storage.getItem('sideleaf.reading-lines.v1'))['book-1'].zheng;
  assert.equal(line.current, 120);
